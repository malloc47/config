# Installs a patched xterm-ghostty terminfo entry. Useful on any host
# that might be SSHed into from a Ghostty terminal. The upstream terminfo
# ships colors#256; this patches it to colors#16777216 so Emacs's C-level
# init_tty enables truecolor in emacsclient -t.
{
  config,
  lib,
  pkgs,
...
in
{
  options.programs.ghostty-terminfo = {
    enable = lib.mkEnableOption "patched xterm-ghostty terminfo install (truecolor)";
  };
